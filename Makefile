...
quick-start: setup install test-basic health-check ## Complete quick setup and test
	@echo "$(GREEN)✅ SmartArb Engine ready to use!$(NC)"
...
check-system: ## Check system prerequisites
	...
	@echo "Python 3.11+: $(shell $(PYTHON) --version | grep -q "Python 3.1[1-9]" && echo "$(GREEN)✓$(NC)" || echo "$(RED)✗$(NC)")"
	@echo "Git: $(shell command -v git >/dev/null 2>&1 && echo "$(GREEN)✓$(NC)" || echo "$(RED)✗$(NC)")"
	@echo "Make: $(shell command -v make >/dev/null 2>&1 && echo "$(GREEN)✓$(NC)" || echo "$(RED)✗$(NC)")"
	@echo "Curl: $(shell command -v curl >/dev/null 2>&1 && echo "$(GREEN)✓$(NC)" || echo "$(RED)✗$(NC)")"
	@if [ "$(IS_RPI)" = "true" ]; then \
		echo "$(YELLOW)Raspberry Pi detected - checking RPi-specific tools...$(NC)"; \
		echo "vcgencmd: $(shell command -v vcgencmd >/dev/null 2>&1 && echo "$(GREEN)✓$(NC)" || echo "$(RED)✗$(NC)")"; \
		echo "GPIO access: $(shell [ -w /dev/gpiomem ] && echo "$(GREEN)✓$(NC)" || echo "$(YELLOW)⚠$(NC)")"; \
	fi
...
setup-git-hooks: ## Setup Git pre-commit hooks
	@if [ -f .pre-commit-config.yaml ]; then \
		...
		echo "$(YELLOW)⚠ .pre-commit-config.yaml not found - skipping Git hooks$(NC)"; \
	fi
...
create-config: ## Create configuration files from templates
	...
	@if [ ! -f config/settings.yaml ]; then \
		...
		echo "$(GREEN)✓ Created config/settings.yaml$(NC)"; \
	fi
